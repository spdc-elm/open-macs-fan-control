...
- [ ] 2.3 Compare reported temperatures and RPM values against an external reference on the target machine
...
- [ ] 3.4 Verify that the requested write produces an observable RPM change on the target machine
- [ ] 3.5 Verify that handled shutdown and handled error paths return fan control to the default automatic mode
