...
Define the long-lived behavior for a macOS menu bar surface that exposes live aggregate CPU and GPU temperatures plus fan RPM telemetry at a glance without requiring the CLI to remain open.

## Requirements

### Requirement: Menu bar extra shows live thermal summary
The system MUST provide a macOS menu bar extra that shows a compact live summary of aggregate CPU temperature, aggregate GPU temperature, and fan RPM telemetry, rendered as a custom multi-column image label via `ImageRenderer`.
...
- **THEN** the menu bar extra MUST render a multi-column compact summary using readable labels (`CPU`, `GPU`, `Fan`) and a colored status indicator, without requiring the user to open a terminal window
